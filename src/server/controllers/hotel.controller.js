...
exports.register = (req, res) => {
  ...
  const {name, email, location, website, phone, password } = req.body
  const newHotelObj = { name, email, location, website, phone, password }
...
exports.update = async (req, res) => {
  try {
    ...
    const updatedUser = await Hotel.findByIdAndUpdate(req.params.id, newDetails, { new: true } ).exec()
    if (!updatedUser) {
      return res.status(400).send({
        success: false,
        message: 'Invalid User ID!'
